...
if not open_api_key:
    raise RuntimeError(
        "The OPENAI_KEY environment variable was not found. Please set it in your environment or .env file."
    )
...
if len(sys.argv) > 1:
    ...
    raise RuntimeError(
        "Document path not specified. Please provide the path as a command line argument or set the DOCUMENT_PATH environment variable."
    )
...
def remove_duplicate_questions(qa_pairs, threshold=0.9):
    ...
    cleaned_qa_pairs = [
        pair for idx, pair in enumerate(qa_pairs) if idx not in to_remove
    ]
