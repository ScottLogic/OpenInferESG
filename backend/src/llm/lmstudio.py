...
class LMStudio(LLM):
    ...
    async def chat(self, model, system_prompt: str, user_prompt: str, return_json=False) -> str:
        logger.debug("Called LMStudio llm. Waiting on response with prompt {0}.".format(
        ...
        if return_json:
            system_prompt = system_prompt + "\nIMPORTANT: You must respond with valid JSON only. " \
                                            "Format your entire response as a proper JSON object."
                ...
                async with session.post(url, json=payload, headers=headers) as response:
                    ...
                    if return_json:
                        ...
                        try:
                            ...
                            _ = json.loads(cleaned_content)  # Just validate, no need to store
                            ...
                            try:
                                ...
                                if json_match:
                                    ...
                                    _ = json.loads(potential_json)  # Just validate, no need to store
